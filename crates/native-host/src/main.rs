...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    loop {
        ...
        match input.read_exact(&mut header) { Ok(()) => {}, Err(error) if error.kind() == io::ErrorKind::UnexpectedEof => break, Err(error) => return Err(error.into()) }
        let size = u32::from_le_bytes(header) as usize;
        if size > MAX_PAYLOAD_BYTES { return Err("native message exceeds limit".into()); }
